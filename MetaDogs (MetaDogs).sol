...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0xe4CAa7578b1d1017408bc971246060c9f801465d;
...
contract MetaDogs is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0xB89441e53779891053984b051a81D5a3F1AB07A9;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "MetaDogs";
    string private _symbol = "MetaDogs";
...
function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
 ...
 function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= 50, "Reflection can not be more than 50 (5%)");
        _taxFee = taxFee;
    }
    function setDevelopmentFeePercent(uint256 developmentFee) external onlyOwner() {
        require(developmentFee <= 100, "DevelopmentFee can not be more than 100 (10%)");
 ...
 function setSwapAndLiquifyEnabled(bool _enabled) public onlyOwner {
      ...
      function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 10, "LiquidityFee can not be more than 10 (1%)");
